...
def do_op(instructions, opcode, parameter_modes, opcode_idx, stdin, stdout):
    ...
    if opcode == 3:
        input_to_program = next(stdin)
        instructions[get_store_at()] = int(input_to_program)

    elif opcode == 4:

        if not parameter_modes:
            stdout.append(instructions[get_parameter()])
        else:
            if parameter_modes == 1:
                stdout.append(get_parameter())
            else:
                stdout.append(instructions[get_parameter()])
...
def intcode_program(filename,stdin):
    ...
    while True:
        ...
        op_result = do_op(instructions, opcode, parameter_modes, opcode_idx, stdin, stdout)
